...
SuperPoint::SuperPointRet SuperPoint::RunSession(const cv::Mat& input_image)
{
  if (input_image.empty())
    throw std::runtime_error("session input image is empty");

  if (std::min(input_image.rows, input_image.cols) < options_->border * 3.0f)
                                    ...
                                    const float& yscale, SuperPointRet& result)
{
  ...
  cv::Mat output_mat0(options_->output_dims0[1], options_->output_dims0[2] * options_->output_dims0[3], CV_32F,
                      output0);
...
SuperPoint::FastNMS(std::vector<cv::Point>& locations, const cv::Mat& confidence)
{
  ...
  std::for_each(std::execution::par_unseq, indices.begin(),
                indices.end(), [&](const int& idx) { flag.at<int8_t>(locations[idx]) = 1; });
...
{
  const int width = options_->output_dims1[3];
  const int height = options_->output_dims1[2];
  ...
  {
    SPDLOG_ERROR("bilinearInterpolate: coordinates out of bounds. "
                 "x={:.2f}, y={:.2f}, x1={}, y1={}, width={}, height={}",
                 x, y, x1, y1, width, height);
    throw std::runtime_error("bilinearInterpolate: coordinates out of bounds");
  }
...
SuperPoint::OptionsPtr SuperPoint::Options::CreateFromYaml(const std::string& yaml_path)
{
  ...
  if (!fs.isOpened()) { throw std::runtime_error("Cannot open YAML file: " + yaml_path); }
  ...
  if (options->border < 8)
  {
    SPDLOG_WARN("Border is too small, set to 8");
    options->border = 8;
  }
  ...
  for (const auto& node : output_dims_node)
  {
    for (const auto& dim_node : node)
    {
      int dim;
      dim_node >> dim;
      if (output_index == 0) { options->output_dims0.push_back(dim); }
      else { options->output_dims1.push_back(dim); }
    }
    output_index++;
  }
...
void SuperPoint::Options::UpdatePointers()
{
  input_names.clear();
  output_names.clear();
